...
def parse_args():
    ...
    parser.add_argument(
        '--output-dir',
        default='/opt/data/private/jqx/mulframe_pcb/anchor_result',
        type=str,
...
class HTCAnchorOptimizer:
    def __init__(self,
        ...
        self.out_dir = out_dir
        bbox_whs, img_shapes = self.get_whs_and_shapes()
        # ratios是实际图片shape/训练input_shap设置参数
        ratios = img_shapes.max(1, keepdims=True) / np.array([input_shape])
        self.iters = iters
    ...
    def kmeans_anchors(self):
        self.logger.info(
            f'Start cluster {self.num_anchors} htc anchors with K-means...')
    ...
    def get_zero_center_bbox_tensor(self):
        ...
        Returns:
            Tensor: Tensor of bboxes with shape (num_bboxes, 4)
            in [xmin, ymin, xmax, ymax] bbox format.
        """
        whs = torch.from_numpy(self.bbox_whs).to(
            self.device, dtype=torch.float32)
        bboxes = bbox_cxcywh_to_xyxy(
    ...
    def save_result(self, anchors, path=None):
        anchor_results = []
        save_file_name = 'anchor_optimize_result.json'
        for w, h in anchors:
            anchor_results.append([round(w), round(h)])
        self.logger.info(f'Anchor optimize result:{anchor_results}')
        if path:
            json_path = osp.join(path, save_file_name)
            mmcv.dump(anchor_results, json_path)
            self.logger.info(f'Result saved in {json_path}')
...
def newmain():
    ...
    scales = cfg.model.rpn_head.anchor_generator.scales
    ratios = cfg.model.rpn_head.anchor_generator.ratios
    strides = cfg.model.rpn_head.anchor_generator.strides
    num_anchors = len(strides) * len(ratios) * len(scales)

    train_data_cfg = cfg.data.train
    while 'dataset' in train_data_cfg:
        train_data_cfg = train_data_cfg['dataset']
    print("--------build dataset from config file:{}--------------".format(train_data_cfg))
    dataset = build_dataset(train_data_cfg)

    if args.algorithm == 'k-means':
        optimizer = HTCAnchorOptimizer(
            dataset=dataset,
            input_shape=input_shape,
            device=args.device,
            num_anchors=num_anchors,
