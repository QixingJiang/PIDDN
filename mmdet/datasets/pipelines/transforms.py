...
class Resize:
    ...
    def __init__(self,
                 ...
                 backend='cv2',
                 override=False):
        # TODO: add the self.random & self.np_random
    ...
    def _random_scale(self, results):
        ...
        elif len(self.img_scale) == 1:  # Just use the fixed single scale
...
@PIPELINES.register_module()
class TempRandomReverse:
    def __init__(self, reverse_ratio=None):
        if isinstance(reverse_ratio, float):
            assert 0 <= reverse_ratio <= 1
        elif reverse_ratio is None:
            pass
        else:
            raise ValueError('reverse_ratio must be None or float')
        self.reverse_ratio = reverse_ratio
    def __call__(self, results):
        """
            random reverse 0-255
        """
        if random.random() < self.reverse_ratio:
            for key in results.get('img_fields', ['img']):
                img = results[key]
                # 反转
                img = 255 - img
...
@PIPELINES.register_module()
class SeedRandomFlip:
    def __init__(self, flip_ratio=None, direction='horizontal', seed=None):
        if isinstance(flip_ratio, list):
            assert mmcv.is_list_of(flip_ratio, float)
            assert 0 <= sum(flip_ratio) <= 1
        elif isinstance(flip_ratio, float):
    ...
    def __call__(self, results):
        ...
        if 'flip' not in results:
            ...
            # random operation here!
            cur_dir = self.np_random.choice(direction_list, p=flip_ratio_list)
            results['flip'] = cur_dir is not None
        if 'flip_direction' not in results:
            results['flip_direction'] = cur_dir
        if results['flip']:
...
class SeedRandomCrop:
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            margin_h = max(img.shape[0] - crop_size[0], 0)
            margin_w = max(img.shape[1] - crop_size[1], 0)
            # 这个offset_h和w决定了 crop region
            offset_h = self.np_random.integers(0, margin_h + 1)
            offset_w = self.np_random.integers(0, margin_w + 1)
            crop_y1, crop_y2 = offset_h, offset_h + crop_size[0]
            crop_x1, crop_x2 = offset_w, offset_w + crop_size[1]
    ...
    def _get_crop_size(self, image_size):
        ...
        if self.crop_type == 'absolute':
            ...
            assert self.crop_size[0] <= self.crop_size[1]
            crop_h = self.np_random.randint(
                min(h, self.crop_size[0]),
                min(h, self.crop_size[1]) + 1)
            crop_w = self.np_random.randint(
                min(w, self.crop_size[0]),
                min(w, self.crop_size[1]) + 1)
            return crop_h, crop_w
        elif self.crop_type == 'relative':
            ...
            crop_size = np.asarray(self.crop_size, dtype=np.float32)
            crop_h, crop_w = crop_size + self.np_random.rand(2) * (1 - crop_size)
            return int(h * crop_h + 0.5), int(w * crop_w + 0.5)
...
class SeedRandomAffine:
    ...
    def __call__(self, results):
        ...
        # Rotation
        rotation_degree = self.np_random.uniform(-self.max_rotate_degree,
                                         self.max_rotate_degree)
        rotation_matrix = self._get_rotation_matrix(rotation_degree)

        # Scaling
        scaling_ratio = self.np_random.uniform(self.scaling_ratio_range[0],
                                       self.scaling_ratio_range[1])
        scaling_matrix = self._get_scaling_matrix(scaling_ratio)

        # Shear
        x_degree = self.np_random.uniform(-self.max_shear_degree,
                                  self.max_shear_degree)
        y_degree = self.np_random.uniform(-self.max_shear_degree,
                                  self.max_shear_degree)
        shear_matrix = self._get_shear_matrix(x_degree, y_degree)

        # Translation
        trans_x = self.np_random.uniform(-self.max_translate_ratio,
                                 self.max_translate_ratio) * width
        trans_y = self.np_random.uniform(-self.max_translate_ratio,
                                 self.max_translate_ratio) * height
        translate_matrix = self._get_translation_matrix(trans_x, trans_y)
