...
@PIPELINES.register_module()
class LoadTestImageFromGroup:
    """Just load the test image from group.
    The Group is often a list of images filepath, the test image is at group[0] by default.
    So we just load the first image of the group and encode it for following pipeline.
    """
    ...
    def __call__(self, results):
        ...
        if self.file_client is None:
            self.file_client = mmcv.FileClient(**self.file_client_args)
        if results['img_prefix'] is not None:
            # the test image postfix is 0 and is the first image in the group list by default.
            group_image_list = results['img_info']['group_image_list']
            test_filename = osp.join(results['img_prefix'], group_image_list[0])
        else:
            pass

        img_bytes = self.file_client.get(test_filename)
        img = mmcv.imfrombytes(
            img_bytes, flag=self.color_type, channel_order=self.channel_order)
        if self.to_float32:
            img = img.astype(np.float32)

        results['filename'] = test_filename
        results['ori_filename'] = test_filename
        results['img'] = img
        results['img_shape'] = img.shape
        results['ori_shape'] = img.shape
        results['img_fields'] = ['img']
...
@PIPELINES.register_module()
class LoadImageFromGroupWithTemplate:
    """Load an image from file with template image.
    ...
    def __call__(self, results):
        ...
        if self.file_client is None:
            self.file_client = mmcv.FileClient(**self.file_client_args)
        if results['img_prefix'] is not None:
            # the test image postfix is 1 and is the first in the group list
            # the temp image postfix is 2 and is the second in the group list
            group_image_list = results['img_info']['group_image_list']
            test_filename = osp.join(results['img_prefix'], group_image_list[0])
            template_filename = osp.join(results['img_prefix'], group_image_list[1])
        else:
            pass
        img_bytes = self.file_client.get(test_filename)
        ...
        img = mmcv.imfrombytes(
            img_bytes, flag=self.color_type, channel_order=self.channel_order)
        template_img = mmcv.imfrombytes(
            template_img_bytes, flag=self.color_type, channel_order=self.channel_order)
        if self.to_float32:
            img = img.astype(np.float32)
        ...
        results['ori_filename'] = test_filename
        results['img'] = img
        # add the template image (np.ndarray)
        results['template_img'] = template_img
        results['img_shape'] = img.shape
        results['ori_shape'] = img.shape
        results['img_fields'] = ['img', 'template_img']
        # mmcv.imshow(img, win_name='test image', wait_time=0)
        # mmcv.imshow(template_img, win_name='template image', wait_time=0)
        return results
