...
class PAAHead_mydfl4(ATSSHead):
    ...
    def __init__(self,
        ...
        self.reg_max = 16
        super(PAAHead_mydfl4, self).__init__(*args, **kwargs)
    ...
    def loss(self,
        ...
        pos_losses_list, consistency = multi_apply(self.get_pos_loss, anchor_list,
        ...
        # consistency_flatten = torch.cat(consistency, 0).view(-1)
        ...
        if num_pos:
            ...
            # pos_consistency_flatten = consistency_flatten[pos_inds_flatten]
            # temp_con = 1.5 - pos_consistency_flatten
...
def cluster_nms(multi_bboxes,
    ...
    for i in range(200):
        A=B
        maxA,_ = A.max(dim=1)
        E = (maxA <= iou_thr).float().unsqueeze(2).expand_as(A)
        B=iou.mul(E)
        if A.equal(B)==True:
            break
