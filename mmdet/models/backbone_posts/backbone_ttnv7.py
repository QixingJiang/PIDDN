...
class BackbonePostTTNv7(BaseModule):
    def __init__(self,
                 in_channels=[96, 192, 384, 768],
                 final_norm=False,
                 init_cfg=None):
        super(BackbonePostTTNv7, self).__init__(init_cfg)
        self.num_feats = len(in_channels)
        self.channel_attentions = nn.ModuleList()
        self.spatial_attentions = nn.ModuleList()
        self.convs = nn.ModuleList()
        self.in_channels = in_channels
        self.weight_a = nn.ModuleList()
        self.weight_b = nn.ModuleList()
        for i in range(self.num_feats):
            ...
            self.weight_a.append(AdaptiveWeight(1.0))
            self.weight_b.append(AdaptiveWeight(1.0))
    ...
    def forward(self, inputs):
        ...
        for i in range(num_lvl):
            ...
            output.append(self.weight_a[i](test_feat) - self.weight_b[i](result))
