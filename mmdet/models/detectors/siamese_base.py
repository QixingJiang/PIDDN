...
class SiameseBaseDetector(BaseModule, metaclass=ABCMeta):
    """Base class for detectors."""

    def __init__(self, init_cfg=None):
        super(SiameseBaseDetector, self).__init__(init_cfg)
        self.fp16_enabled = False

    @property
    def with_img_post(self):
        return hasattr(self, 'img_post') and self.img_post is not None

    @property
    def with_backbone_post(self):
        return hasattr(self, 'backbone_post') and self.backbone_post is not None

    @property
    def with_neck_post(self):
        return hasattr(self, 'neck_post') and self.neck_post is not None
    ...
    def forward_train(self, imgs, img_metas, **kwargs):
        ...
        batch_input = imgs[0]
        batch_input_shape = tuple(batch_input[0].size()[-2:])
        # each_group is a set of some images
        for group in img_metas:
            for img_meta in group:
    ...
    def train_step(self, data, optimizer):
        ...
        losses = self(**data)        # self(**data) is equal to __call__()
